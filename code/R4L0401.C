...
void main()
{
int gd=VGA,gm=VGAHI,errorcode,area,i,j,play,rndplay,rac,r,ch,chh,n=1,speed,sp=1,si=1,carx,cary,gear,aspeed,booster,drumx,drumy,drumv,drumx1,drumy1,drumx2,drumy2,drumx3,drumy3,spp,coin,life_;
char *buff;
int fire;
int soun;
char *fir;
...
//geting front police car5 image enemy object**********************************************
pfcar();
area=imagesize(195,145,255,282);
...
for(rac=1;rac<33;rac++)
{
		if(kbhit())
		{
		putimage(drumx,drumy+=20,car2buf,XOR_PUT);
		roadspeed(speed);
		fflush(stdin);
		ch=bioskey(0);
		...
		else
		{
			...
			{
			putimage(drumx,drumy+=20,car2buf,XOR_PUT);     //dreum
			roadspeed(100);
...
for(rac=1;rac<33;rac++)
{
		if(kbhit())
		{
		putimage(drumx,drumy+=20,pcar4buf,XOR_PUT);
		roadspeed(speed);
		fflush(stdin);
		ch=bioskey(0);
		...
		else
		{
			...
			{
			putimage(drumx,drumy+=20,pcar4buf,XOR_PUT);     //dreum
			roadspeed(100);
...
for(rac=1;rac<33;rac++)
{
		...
		{
		putimage(drumx1,drumy1+=20,car2buf,XOR_PUT);
		putimage(drumx,drumy-=20,fcar5buf,XOR_PUT);
		roadspeed(speed);
		fflush(stdin);
		ch=bioskey(0);
		putimage(drumx1,drumy1,car2buf,XOR_PUT);
		putimage(drumx,drumy,fcar5buf,XOR_PUT);
	     ...
	     //		putimage(carx,cary,buff,XOR_PUT);
		  ...
		  //	putimage(carx,cary,buff,XOR_PUT);
			...
			putimage(carx+32,cary-24,fir,XOR_PUT);
			nosound();
			if(drumx1>carx && drumx1<carx+75 || drumx1+72>carx && drumx1+72<carx+75 || drumx>carx && drumx<carx+75 || drumx+60>carx && drumx+60<carx+75)
			{
			rac=1000;
			}
			}
		...
		else
		{
			...
			{
			putimage(drumx1,drumy1+=20,car2buf,XOR_PUT);     //dreum
			putimage(drumx,drumy-=20,fcar5buf,XOR_PUT);
			roadspeed(100);
			putimage(drumx1,drumy1,car2buf,XOR_PUT);  //drum
			putimage(drumx,drumy,fcar5buf,XOR_PUT);
			}

		fflush(stdin);
		}
			if(drumx1>carx && drumx1<carx+75 || drumx1+72>carx && drumx1+72<carx+75)
			{
			if(drumy1+149>=cary)
			{
			rac=3000;
