...
//geting trucks image enemy objects*********************************************************
truck1();
area=imagesize(190,130,260,298);
truck1buf=malloc(area);
getimage(190,130,260,298,truck1buf);
putimage(190,130,truck1buf,XOR_PUT);
//geting feul drum's image**********************************************
drumx=250;
drumy=15;
feuldrum(drumx,drumy);
area=imagesize(170,0,220,30);
feulb=malloc(area);
getimage(170,0,220,30,feulb);
putimage(170,0,feulb,XOR_PUT);
...
if(rndplay==1)
{
	      //truck1 fall standerd truck               single truck in road
drumv=random(390);
	if(drumv>180)
	{
	 drumx=drumv;
...
for(rac=1;rac<33;rac++)
{
		if(kbhit())
		{
		putimage(drumx,drumy+=20,truck1buf,XOR_PUT);
		roadspeed(speed);
		fflush(stdin);
		ch=bioskey(0);
		putimage(drumx,drumy,truck1buf,XOR_PUT);
		...
		else
		{
			...
			{
			putimage(drumx,drumy+=20,truck1buf,XOR_PUT);     //dreum
			roadspeed(100);
			putimage(drumx,drumy,truck1buf,XOR_PUT);  //drum
			}
...
{
	      //car6 fall   pfcar             single car in road
drumv=random(400);
	if(drumv>180)
	{
	 drumx=drumv;
...
for(rac=1;rac<=50;rac++)
{
		if(kbhit())
		{
		putimage(drumx,drumy-=20,pfcar6buf,XOR_PUT);
		roadspeed(speed);
		fflush(stdin);
		ch=bioskey(0);
		putimage(drumx,drumy,pfcar6buf,XOR_PUT);
		...
		else
		{
			...
			{
			putimage(drumx,drumy-=20,pfcar6buf,XOR_PUT);     //dreum
			roadspeed(100);
			putimage(drumx,drumy,pfcar6buf,XOR_PUT);  //drum
			}
		...
		fflush(stdin);
			...
			if(drumx>carx && drumx<carx+75 || drumx+60>carx && drumx+60<carx+75)
			{
			if(drumy>=cary && drumy<=cary+136 || drumy+137>=cary && drumy+137<=cary+136)
			{
			life_--;
			lifee(life_);
			rac=50;
...
for(rac=1;rac<33;rac++)
{
		if(kbhit())
		{
		putimage(drumx,drumy+=20,feulb,XOR_PUT);
		roadspeed(speed);
		fflush(stdin);
		ch=bioskey(0);
		...
		else
		{
			...
			{
			putimage(drumx,drumy+=20,feulb,XOR_PUT);     //dreum
			roadspeed(100);
		...
		fflush(stdin);
			...
			if(drumx>carx && drumx<carx+75 || drumx+48>carx && drumx+48<carx+75)
			{
			if(drumy+30>=cary)
			{
			printf("\a");
...
{
//double  fall truck1 and feuldrum      with front car    double sided carfall
drumv=random(285);
	if(drumv>180)
	{
	drumx=drumv;
	drumx1=(drumx+70)+random(103);
	}
	else
	{
	drumx=200;
...
for(rac=1;rac<33;rac++)
{
		...
		{
		putimage(drumx,drumy+=20,truck1buf,XOR_PUT);
		putimage(drumx1,drumy1+=20,feulb,XOR_PUT);
		roadspeed(speed);
		fflush(stdin);
		ch=bioskey(0);
		putimage(drumx,drumy,truck1buf,XOR_PUT);
		putimage(drumx1,drumy1,feulb,XOR_PUT);
	     ...
	     //		putimage(carx,cary,buff,XOR_PUT);
		  ...
		  //	putimage(carx,cary,buff,XOR_PUT);
			...
			putimage(carx+32,cary-24,fir,XOR_PUT);
			nosound();
			if(drumx>carx && drumx<carx+75 || drumx+70>carx && drumx+70<carx+75 || drumx1>carx && drumx1<carx+75 || drumx1+60>carx && drumx1+60<carx+75)
			{
			rac=1000;
			}
			}
...
{
//double  fall      with front car  car1 & pfcar   double sided carfall
drumv=random(285);
	if(drumv>180)
	{
	drumx=drumv;
...
for(rac=1;rac<33;rac++)
{
		if(kbhit())
		{
		putimage(drumx,drumy+=20,car1buf,XOR_PUT);
		putimage(drumx1,drumy1-=20,pfcar6buf,XOR_PUT);
		roadspeed(speed);
		fflush(stdin);
		ch=bioskey(0);
		...
		else
		{
			...
			{
			putimage(drumx,drumy+=20,car1buf,XOR_PUT);     //dreum
			putimage(drumx1,drumy1-=20,pfcar6buf,XOR_PUT);
			roadspeed(100);
		...
		fflush(stdin);
			...
			if(drumx1>carx && drumx1<carx+75 || drumx1+60>carx && drumx1+60<carx+75)
			{
			if(drumy1>=cary && drumy1<=cary+137 || drumy1+136>=cary && drumy1+136<=cary+137)
			{
			rac=3000;
			...
			else if(drumx>carx && drumx<carx+75 || drumx+60>carx && drumx+60<carx+75)
			{
			if(drumy+148>=cary)
			{
			rac=3000;
